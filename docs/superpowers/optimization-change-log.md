...
| 文档 diff | 无 Hub 契约文档改动：本次三项都不改变工具的**入参约束**或 `risk` 分级，改变的是资源用量与内部路由机制。插件自有契约同步 1 处：`docs/decisions/ADR-003-panel-alert-embedding-via-local-proxy.md` 的决策段新增一条「打到代理根路径的子资源请求按 `Referer` 归属、绝不依赖环境态」，写明同源校验、`Referrer-Policy: same-origin` 的必要性、归属不出来即裸 404，并追记 2026-08-13 之前的 `Path=/` cookie 形态、它为何会串台、以及为何 `Path=/instances/<id>` 修不好。ADR-003 原有的鉴权条目说「every embed URL carries and every request must present」，只描述了 URL 一种呈递方式，本次补上第二种。其余取舍与理由密集写在代码 JSDoc（`parseEmbedReferrer` 的「为什么不是 cookie」、`DEFAULT_EMBED_PROXY_LIMITS` 的「两个 buffer 上限为什么必须成对」、`QueryRateLimiter` 的「这不是授权边界」、`planQueryLimits` 的逐条绕过说明）与三条提交信息正文中 |
| protocolVersion | 不变（Bridge 1 / Hub 2）。本仓是插件侧，未参与协议版本 |
...
| 提交 | `3e94826`（G4 referrer 归属，2 文件）、`671541b`（G5 超时与并发，2 文件）、`7502f68`（G6 查询计量，7 文件）、`7401bdc`（ADR-003 同步，1 文件），另本条台账 1 条写入 hub 仓。均在分支 `chore/at-series-optimization-phase0`，**`git add` 与 `git commit` 两侧全部带 pathspec**，每次提交后以 `git show --stat` 复核文件数（2 / 2 / 7 / 1，与预期逐一相符）。未推送远程，未改 `package.json` / `package-lock.json`。两仓中他人未提交的在制品（grafana 仓 13 个 `M`、hub 仓 3 个 `M` 与 4 个 `??`）全程未被波及 |
...
### 2026-08-13 · P6-T · AT Terminal 性能批次：输出传输、高亮去 O(m²)、lrzsz、未同步编辑、上传校验、非阻塞通知

| 字段 | 内容 |
|---|---|
| 仓库 | at-terminal-series |
| 动机 | 审计 T7–T12 六个 P1。T7：终端输出以逐字节 JSON 数组跨 postMessage 传输且不合并；T8：语义高亮去重在匹配数上 O(m²) 且默认开启；T9：lrzsz 检测吞掉整个 chunk 而功能本身是空壳；T10：未同步的远程编辑在关闭标签页时被静默 `rm(force)`；T11：上传校验用同步 IO 阻塞扩展宿主并把刚上传的文件重下一遍；T12：`showTimedNotification` 把调用方阻塞整个提示时长 |
| 代码 diff | T7 新增 `src/webview/TerminalOutputBatcher.ts`（66 行）、`src/webview/TerminalPanel.ts:44,155,181,191-201,213-217`、`webview/terminal/output.ts:16-41`；T8 `webview/terminal/semanticHighlight.ts:1-118`（`mergeNonOverlappingMatches` 折叠 + `MAX_HIGHLIGHT_TEXT_LENGTH` / `MAX_HIGHLIGHT_MATCHES` 阈值）；T9 删除 `src/lrzsz/LrzszDetector.ts`、`src/lrzsz/LrzszTransfer.ts`，`src/webview/TerminalPanel.ts:191-201`；T10 `src/sftp/SftpEditSessionManager.ts:41,214-231,447,462-471`；T11 `src/sftp/SftpEditSessionManager.ts:2,14-18,325-370`；T12 `src/utils/notifications.ts:8-36` 与 17 个调用点（`src/extension.ts` 12 处、`src/sftp/VscodeTransferReporter.ts` 2 处、`src/sftp/SftpEditSessionManager.ts` 2 处、`src/webview/TerminalPanel.ts` 1 处）。测试：新增 `test/webview/TerminalOutputBatcher.test.ts`、`test/webview/TerminalSemanticHighlight.test.ts`，改写 `test/webview/TerminalOutputMessage.test.ts`、`test/webview/TerminalPanel.test.ts`、`test/sftp/SftpEditSessionManager.test.ts`、`test/sftp/VscodeTransferReporter.test.ts`、`test/utils/notifications.test.ts`，删除 `test/lrzsz/LrzszDetector.test.ts`，`test-fixtures/vscode.ts` 的 `StatusBarItem.command` 放宽到 `string | Command` |
| 契约影响 | 否 —— 六项全部在插件内部：webview postMessage 是插件私有通道（不在 `v1.md`/`v2.md` 覆盖范围内），SFTP 编辑会话与通知是本地 UI 行为，未触及 Bridge 线协议、registry、工具 schema 或 Hub 暴露形状 |
| 文档 diff | 本条目；`README.md:169`、`README-base.md:70-72,205-207` 删除 lrzsz 特性描述（T9 移除了该链路，留着就是虚假承诺） |
| protocolVersion | 不变（Bridge 1 / Hub 2） |
| 插件需跟改 | 是（同步而非依赖）：`at-jumpserver-series/webview/terminal/output.ts` 与本仓副本在改动前逐字节相同，其并行任务 J8 应采用同一方案——`outputBytes` 的 payload 改 base64 字符串、`atob` 解码、删除 `isByteArray` 逐元素校验，宿主侧 8ms / 64KB 合并。两边保持一致，阶段 5 抽共享包时才能直接合并而不必先对齐分歧。**未跨仓 import**，本仓只改本仓副本 |
| 核心不变量 | 逐条核对：**INV-1** 未触及——未改 `McpConfigInstaller` / `hubSync` / registry 写入，`test/mcp/McpConfigInstaller.test.ts`（沙箱外）与 `McpServerManifest.test.ts` 全绿，IDE 里仍只有一条 `AT Series`；**INV-2** 未触及——未新增任何插件侧 MCP 入口，`rg 'registerTool\|languageModelTools' src package.json` 零匹配，T9 只是**删除**了一条本地 UI 链路而非新增入口；**INV-3** 未触及——本批次未改 hub 仓任何源码（仅按任务书重建 `dist/` 产物以避免误导性绿）；**INV-4** 未触及——未改 `AT_SERIES_TOOL_DISCOVERY` 默认值或阈值；**INV-5** 未触及——未改 `BridgeServer` / `toolCatalog` / 9 个工具的注册与路由，selection 语义原样；**INV-6** 未触及——五个 Hub 元工具不在本仓 |
| 验证 | **沙箱外**（`required_permissions: ["all"]`，先 `cd at-series-mcp-hub && npm run build && npm run build:hub` 重建产物）：`npm run typecheck` 零诊断；`npm test` **62 文件 / 456 用例全绿**（基线 61 文件 / 421 用例；净 +35 = 新增 37 − 删除 2 个 lrzsz 用例）。**沙箱内**：typecheck 同样零诊断，`npm test` 3 failed / 453 passed，失败全部是 `test/mcp/McpConfigInstaller.test.ts` 的 `EPERM: operation not permitted, mkdir '/var/folders/.../at-terminal-mcp-installer-*/.cursor'`，属沙箱写权限限制，与本批次改动无关（沙箱外同样三条通过）。**基准数据**见下方散文段 |
| 提交 | `255b494`（T7）、`77b5ebb`（T8）、`b6d48c6`（T9）、`9c1db08`（T10）、`4625812`（T11）、`e8d4dbd`（T12），分支 `chore/at-series-optimization-phase0`，未推送。`git add` 与 `git commit` 两侧均带显式 pathspec，未使用 `.` / `-A` / `-u`；`package.json` / `package-lock.json` 未纳入任何一次提交 |

**T7 的量化收益与为什么合并阈值取 8ms / 64KB。** 一个 32KB 的 chunk 在旧路径上要做三件事：`[...data]` 展开成 32768 个 number、序列化成 107,646 字节 JSON（3.29 倍放大）、webview 侧再用 `Array.every` 逐元素校验一遍。改 base64 后线上体积 43,727 字节（1.33 倍），宿主编码 0.97ms → 0.02ms，webview 解码 0.32ms → 0.05ms。把一次 4 MB 的 `cat` 按 8KB 包回放：**512 条消息 → 64 条，13.2 MiB → 5.3 MiB，宿主 CPU 79.1ms → 2.4ms**。阈值的依据是两端各卡一头：**8ms 是 60Hz 一帧（16.7ms）的一半**，所以被推迟的输出仍落在它产生的那一帧内，用户感知不到延迟；**64KB 封顶单条消息的解码代价约 0.1ms**，避免高吞吐时合并出一条大到自身就要卡帧的消息。两者取先到者。**删掉 `isByteArray` 不是省事**：payload 来自可信的扩展宿主、走的是 webview 私有通道，发送方不可能是攻击者，为此付 O(n) 是在防一个不存在的威胁模型。

**T8 的基准数据，以及去重等价性是怎么证明的。** 生成 CSV 文本实测：4KB / 882 匹配 1.34ms → 0.06ms；16KB / 3268 匹配 9.28ms → 0.24ms；**32KB / 6191 匹配 22.26ms → 0.51ms（43.9 倍）**——22ms 发生在渲染线程上，本身就超过一帧预算。加上 8KB / 200 匹配的阈值后，32KB 这一档端到端 23.28ms → ~0ms（直接跳过）。**等价性没有靠「排序后线性扫描」**：那个方案会破坏语义，反例是 `cat /var/error.log`——原实现按规则优先级先到先得，`error`（规则 0）先被接受，路径 `/var/error.log`（规则 5）虽然起点更靠左但重叠被拒；按 start 排序后从左往右扫会反过来选中路径。改用**按规则折叠**：单条全局正则的匹配本就有序且互不重叠，所以可以把一条规则的整组匹配用双指针一次并入已接受集合，这与逐个判定完全等价（同组内互不重叠 ⇒ 接受其一不影响其余），复杂度 O(规则数 × m) 而非 O(m log m)。证明分两层：**300 组随机区间的差分测试**对拍一个「先到先得」参考实现（保留在测试文件里，正是被替换掉的那段逻辑），加**三条会让朴素排序扫描选错的文本用例**（路径 vs error、IP vs 数字同起点、URL vs 内嵌数字）。

**T9 为什么选「移除」而不是「配置项默认关闭」。** 配置项只能在「坏」和「关」之间二选一——传输侧 `LrzszTransfer` 只弹一个「waiting for protocol adapter validation」的提示，没有任何实现可供打开；而打开时的行为是 `passthrough: ''` 让 `TerminalPanel` 丢掉整个 chunk，用户敲 `rz` 得到的是终端静默、无传输、且因为回显没回来连中断 ZMODEM 的五次 `Ctrl-X` 都送不出去。留一个默认关闭的开关等于把这个陷阱留在设置面板里等人踩。检测本身也不可靠：它基于单 chunk 的 latin1 字符串，ZMODEM 头跨 TCP 包时检测不到，行为取决于包边界。**真正的 lrzsz 支持是一个功能而不是一处修复**——需要跨 chunk 的帧解码器、中断路径、文件选择器——列为后续项。移除后 zmodem 字节原样进 xterm，至少会话可中断。

**T10 那条把错误行为固化的测试是怎么改写的，以及为什么原预期是错的。** 原用例名为 `deletes cache and unregisters failed sessions on close without replacing the original error`，断言 `existsSync(session.localUri.fsPath) === false`。它把「关闭编辑器 = 同意丢弃未保存的工作」当成了规格，而在上传失败（权限、断线、冲突取消）后，那个缓存文件是用户改动的**唯一副本**——所以这条用例会在数据被销毁的同时保持通过。改写成两条表达新语义的用例：`keeps the cached file and offers a recovery entry when the user keeps an unsynchronized edit`（断言 `promptUnsyncedClose` 被调用、文件仍在、**内容仍是用户写入的那段**、状态栏入口带上了 localUri、原始错误未被覆盖）与 `deletes the cached file when the user discards an unsynchronized edit`；另加 `prompts for a conflicted session after recording why the close interrupted the sync` 覆盖 `failSync` 与新提示的先后关系，以及给干净会话补了 `promptUnsyncedClose` 未被调用的断言。**保护早就被设计过但从未接上**：`promptUnsyncedClose` 在接口和 `createVscodeSftpEditUi` 里都实现了，`hasUnsynchronizedState` 也在，两者都没有任何调用方。

**T11 选了 size + mtime 短路而不是 SHA-256，以及为什么这样更准。** 这条校验要抓的失败是「服务器接受了写入却没有真的改文件」——而那种失败**同样不会推进 mtime**。所以：上传后 stat 的 size 与本地一致、且 mtime 越过了**紧贴上传前**那次 stat 的 mtime，写入就已被证明，可以跳过重下；剩下的是一张有界的安全网——256KB 以内的文件在 mtime 没动时仍做全量字节比对。每次保存的回拉量：1MB 文件 1024KB → 0KB，8MB 文件 8192KB → 0KB。**不选 sha256sum 的两个理由**：一是它需要在 SFTP 编辑路径上引入远程命令通道，而刚落地的 Agent 授权模型正刻意把这个面收窄；二是 macOS / busybox 上没有 `sha256sum`，处理「命令不存在」本身就要一条 `shasum -a 256` / `openssl dgst` 的回退链，为一次校验引入的复杂度超过收益。另一处必须点明的细节：**传入的是上传前那次 stat 而不是 `session.baseRemoteStat`**——后者在「冲突后选择覆盖」时是过期的，用它做 mtime 比较会在我们的写入其实没落地时也判定通过。同步 IO 换成 `fs/promises` 的收益用「宿主还能不能干别的」度量：读 64MB 缓存文件 20 次，旧路径宿主服务了 **0/175** 个计划中的 1ms 时间片（全程冻结），新路径 **127/125**；8MB 档是 0/14 → 17/15。

**T12 全部 17 个调用点的检查结果：没有任何一个依赖阻塞语义。** 5 处本来就写了 `void`（hub sync 失败、bridge 启动失败、MCP 配置更新失败、空闲断连提示、以及 `TransferService` 侧的 `void this.reporter?.notify*`）。其余 12 处分两类：一类是**提示完就 return**（`extension.ts` 拒绝变更的 host key 后 `return false`、MCP 安装/卸载命令的各个分支、跳板机删除被阻止、`runSftpCommand` 的 catch），阻塞只是让 SSH 握手白挂 3 秒、让每次 SFTP 失败白卡 3 秒；另一类是**本就是即发即忘的上报**（`VscodeTransferReporter.notifySuccess/notifyFailure`、编辑会话的 `showSuccess`/`showError`）。`showError` 稍需留意：它在 `failSync` 里被 await，而 T10 之后 `closeSession` 会在 `failSync` 之后弹模态框——旧行为是模态框要等 8 秒错误提示消失才出现，改完是提示与模态框同时呈现，更好。返回类型改成 `void` 而不是「立刻 resolve 的 Promise」，是为了让任何调用点都无法**看起来**在对提示做时序编排。即发即忘意味着没人再接得住失败，所以对已经消失的通知宿主（关闭期）做了吞异常处理，否则它会变成随机调用方里的未捕获拒绝。两条固化阻塞语义的测试（`VscodeTransferReporter` 的「3 秒 / 8 秒」与编辑会话的 8 秒失败提示）改写为断言时长真正的含义——**提示在屏幕上停留那么久，而调用方立即恢复**。

**一处顺带的测试基建修正。** T11 把缓存文件读取从同步换成 `fs/promises` 之后，`SftpEditSessionManager.test.ts` 原来的 `flushPromises`（两次 `await Promise.resolve()`）不再足以让上传链路走完——真实文件读取在 I/O 轮次上完成而不是微任务上，用 0ms 假定时器推进也只是在 timers 阶段空转，256KB 的读取会赶不上。改为 `settleUpload(session)` 直接 await `session.uploadTask`，从「猜要几个 tick」变成「等那件事做完」。

**并发协作说明。** 六次提交 `git add` 与 `git commit` 两侧都带显式路径列表，每次提交后 `git show --stat` 复核文件数（6 / 2 / 7 / 3 / 2 / 8，与预期逐一相符）；`package.json`、`package-lock.json`、`docs/releases/0.3.0.md` 与 `docs/handoffs/` 全程保持他人留下的 ` M` / `??` 状态未被波及。**按前几条台账记下的教训，追加前重新读取了本文件尾部**——首次读取时 806 行，落笔前已是 907 行（期间他人追加了 P4-J/J10 一条），因此本条落在真实末尾而非旧快照位置。L776–788 那个被截断的 P6-H 条目仍在，本条未修改该区域。

### 2026-08-13 · P6-G2 · Grafana Bridge 鉴权前置、常量时间 token 比较与生产构建压缩

| 字段 | 内容 |
|---|---|
| 仓库 | at-grafana-series |
| 动机 | **① Bridge 在鉴权之前就缓冲 2 MiB 请求体。** `handleNodeRequest` 做的第一件事是 `readLimitedBody(request, BRIDGE_MAX_BODY_BYTES)`，而 token 校验在 `createBridgeRequestHandler` 返回的闭包**里面**，也就是 body 读完之后才发生。绑定 `127.0.0.1` 挡住的是网络，挡不住同一台机器上的任何用户态进程——它们本来就在这条边界之内。于是**无需出示任何凭据**，任意本地程序都能让 VS Code 扩展宿主为它每条 socket 持有 2 MiB，而 socket 数量没有上限。**② token 用 `===` 比较（H9 在插件侧的欠账）。** hub 自 P2-C 起就导出了 `timingSafeEqualToken` 与 `createBridgeToken`，`docs/protocol/v1.md` §7.2 也已把常量时间比较写成 MUST，本仓一直没跟上。**③ 生产构建既不压缩又留悬空 sourcemap。** `esbuild.config.mjs` 写死 `minify: false` / `sourcemap: true`，`dist/extension.js` 约 556 KB 未压缩；而 `.vscodeignore` 排除 `**/*.map`，于是 VSIX 里两个 bundle 各带一条指向**不存在文件**的 `//# sourceMappingURL`。两处 esbuild `target` 也未显式指定，由工具链自行决定能发什么语法 |
| 代码 diff | **① 鉴权前置（`29058e6`，2 文件 +348/-20）**：`src/mcp/BridgeServer.ts` **+81/-20**——`handleNodeRequest` 签名增第二参 `token: string`，函数体开头改为 `if (!isAuthorized(request.headers, token)) { respond(response, unauthorizedResponse()); return; }`，`readLimitedBody` 移到该判定之后；新增模块级 `unauthorizedResponse()`（handler 与传输层共用同一个 401 信封，杜绝两处形状漂移）与 `respond(response, result)`（`handleNodeRequest` 里三处手写 `statusCode`/`setHeader`/`end` 收敛成一处，413 与 500 顺带改由 `bridgeError` 构造）；新增导出 `BridgeServerLimits` 与 `DEFAULT_BRIDGE_SERVER_LIMITS`（`requestTimeoutMs: 30_000` / `headersTimeoutMs: 10_000` / `maxConnections: 64`），`BridgeServerOptions` 增可选 `limits`，构造器合并，`start()` 改用 `createServer(options, handler)` 传 `requestTimeout` / `headersTimeout` / **`connectionsCheckingInterval`** 并设 `server.maxConnections`。新增 `test/mcp/BridgeServer.http.test.ts` **+267**（真实 socket 黑盒，7 用例）。**② 常量时间比较（`07eae8a`，3 文件 +89/-8）**：`src/mcp/BridgeServer.ts` **+33/-8**——import 去掉 `randomBytes`、加 `createBridgeToken` 与 `timingSafeEqualToken`；`isAuthorized` 改为 `timingSafeEqualToken(headerValue(...) ?? '', token)`；`headerValue` 增 `typeof first === 'string'` 收窄；`start()` 的 `randomBytes(32).toString('hex')` 改为 `createBridgeToken()`。`test/mcp/BridgeServer.test.ts` **+52**（新增 `describe('BridgeServer token comparison')`，6 用例）、`test/mcp/BridgeServer.http.test.ts` **+12/-1**（新增「token 由 hub 的 minter 铸造」一条）。**③ 生产构建（`865857e`，1 文件 +15/-3）**：`esbuild.config.mjs`——`sourcemap: true` → `sourcemap: watch`，`minify: false` → `minify: !watch`，宿主侧加 `target: 'node18'`，webview 侧加 `target: 'chrome114'` |
| 契约影响 | **否。** 三项都不改 Bridge 线协议：`/health`、`/tools`、`/invoke` 的路径、方法、请求与响应 JSON 形状逐字未变；401 / 413 / 500 三种错误信封改由 `bridgeError` 统一构造，输出字节与改前完全相同（`{"error":{"code":…,"message":…}}`）；registry 字段、publisher、hub sync、installer 零改动；9 个工具名与 `risk` 分级未动。**任务 ② 需要单独说明：它让本仓对齐 `v1.md` §7.2 已经写成 MUST 的常量时间比较，属于「兑现既有规范」而不是「修改规范」，因此契约影响仍填「否」，v1.md 无需 diff。** 三处**插件自有的**可观察行为变化，如实记录：(1) **未鉴权**且 body 超限的请求，此前返回 `413 PAYLOAD_TOO_LARGE`，现在返回 `401 UNAUTHORIZED`——凭据不合格的调用方不再从服务端换到关于自己 body 尺寸的任何信息，已鉴权调用方的 413 行为逐字不变；(2) 配置 token 为空串时，此前「调用方也送空串」会被判定为**通过**（`'' === ''`），现在恒为 401；(3) token 编码由 64 字符 hex 变为 43 字符 base64url，熵同为 32 字节——已核实 hub 侧 `packages/mcp-hub/src/registry/read.ts` 对 `token` 的唯一约束是 `isNonEmptyString`，无字符集或长度校验，故对 Hub 不可见 |
| 文档 diff | 无契约文档改动（理由见上栏）。取舍与理由密集写在代码 JSDoc（`handleNodeRequest` 的「为什么必须先判凭据」、`isAuthorized` 的「`?? ''` 为何是承重的」、`headerValue` 的「为什么要收窄成 `string`」、`BridgeServerLimits` 的「这是失控护栏不是调优旋钮」、`esbuild.config.mjs` 的「为什么 target 要钉在 `engines.vscode` 上」）与三条提交信息正文中。本条台账即为记录 |
| protocolVersion | 不变（Bridge 1 / Hub 2）。本仓是插件侧，不参与协议版本 |
| 插件需跟改 | 否。**但任务 ① 的形状对另两仓有参考价值，值得排查**：任何「先把请求体读进内存、再校验凭据」的本地 HTTP 服务，都是一条无需凭据即可触发的内存放大通道，绑定回环并不能免疫。terminal / jumpserver 的 Bridge 是否同形，未在本任务中排查。任务 ② 对应 H9 表格里「**是**：三插件改用导出的比较函数」，本仓至此完成，另两仓状态未知 |
| 核心不变量 | 已核对 **INV-1..INV-6 未被破坏**。**INV-1**（IDE 的 MCP 配置只能有一条 `AT Series`）：三次提交共 4 个文件，不含 `McpConfigInstaller*` 或其任何调用点，未触碰任何 MCP 配置写入路径。**INV-2**（不得恢复 per-plugin 入口 / `languageModelTools`）：`rg 'languageModelTools|mcp-server\.js' src/ package.json` **0 匹配**，与改动前一致；`package.json` 全程未改。**INV-3**（Hub 内不写死插件工具清单）：hub 仓 `packages/mcp-hub/src/**` 零改动，本次只向 hub 仓追加本条台账。**INV-4**（`AT_SERIES_TOOL_DISCOVERY` 默认 `auto` / 阈值 `20`）：`rg 'AT_SERIES_TOOL_DISCOVERY' src/` **0 匹配**，本仓不写这两个值。**INV-5**（渐进暴露只影响 `tools/list`，selection 不是 ACL）：本次新增的拒绝路径只有两类，都与 selection 无关——401 是**凭据**判定（判据只有 `x-at-series-token` 一个 header，不读工具名、不读 selection 状态），且它本就存在，本次只是把它挪到 body 之前；socket 超时与 `maxConnections` 是**传输层**资源护栏，在 HTTP 请求被解析成工具调用之前就已生效，同样不看工具身份。Hub 侧 `tools/list` 与路由行为逐字未变，未 select 的工具照样可 `tools/call`。**INV-6**（五个 Hub 元工具始终暴露、`risk: read`、在 autoApprove 内）：五个元工具是 Hub 内建，本仓无从触碰；`src/mcp/toolCatalog.ts` 未在本次任何一次提交的文件清单内，9 个工具的 `risk` 分级逐字未变，installer 计算出的 autoApprove 集合不变 |
| 验证 | **前置动作：** 按 OPS-1 的教训，开工前先在 hub 仓执行 `npm run build && npm run build:hub`（`Bundled hub.js (0.3.0) -> dist/hub.js`），避免又一次跑在陈旧 dist 上；本次未发现新的跨仓漂移。**① 任务 1 严格 TDD，两条 RED 各自观测一个独立现象。** 第一条观测「服务端在读完 body 之前是否回话」：起真实 `BridgeServer`（`home` 指向 mkdtemp，port 与 token 从发布出来的 registry 记录里读，与 Hub 的取法一致），裸 socket 发一个**不带 token**、`content-length: 4 MiB` 但只写 1 KiB 就停手的 POST，失败原文 `Error: unauthenticated request with an unfinished body did not settle within 2000ms`——旧实现在等那 4 MiB，永远等不到，2000ms 后被 `within()` 转成可读失败而非 vitest 级超时。第二条用**状态码本身**做示波器：同样不带 token，但把超限 body 完整发完，失败原文 `AssertionError: expected 'HTTP/1.1 413 Payload Too Large' to contain '401'`——413 只有「已经把字节数完整数过一遍」才产生得出来，这就是「body 在鉴权前被读取」的直接证据。修复后两条均绿，且第二条耗时从 2146ms 降到 27ms。**② 超时那两条 RED 是事后反证的，如实记录顺序：** 实现与测试同批写出，随后**临时把 `createServer` 的三个选项换回 `connectionsCheckingInterval: 30_000` 单项**复跑，得到 `Error: slowloris socket did not settle within 3000ms` 与 `Error: authenticated socket with an unfinished body did not settle within 3000ms`，确认这两条测试确实咬得住实现，然后还原。绿态下 300ms/600ms 期限分别在 503ms / 1003ms 收口，与 `connectionsCheckingInterval` 的扫描节奏相符。**③ 任务 2 的 RED 是一条真实的判定翻转：** `expected 200 to be 401`——旧的 `===` 让「配置 token 为空 + 调用方送空串」直接**通过**，即一个没能铸出 token 的 Bridge 是敞开的。**④ 非字符串边界是实测出来需要额外处理的，不是预防性加的：** 先只把 `isAuthorized` 换成 `timingSafeEqualToken(… ?? '', token)` 复跑，得到 `expected 500 to be 401`——`Buffer.from(42, 'utf8')` 抛异常被 handler 的 catch 兜成 500；补上 `headerValue` 的 `typeof` 收窄后回到 401。**⑤ 任务 3 用实际打包结果验证，不靠推断：** 同一份源码分别按新旧配置各跑一次 `npm run build && npm run copy:hub && node scripts/package.mjs`（沙箱外，`required_permissions: ["all"]`——沙箱内 vsce 的 secretlint 会因 `os.cpus().length` 返回 0 崩在 `Expected 'concurrency' to be an integer from 1 and up, got '0'`）。体积：`dist/extension.js` **568,898 → 268,966 字节（−52.7%）**，`dist/webview/grafana-instance-form.js` **2,775 → 1,500 字节（−46.0%）**，`at-grafana-0.1.0.vsix` **233,201 → 199,691 字节（−14.4%）**；VSIX 降幅小得多是因为它本就 deflate 过，且体积由本仓只复制不构建的 `dist/hub.js`（381 KB）主导。两个 VSIX 都解包比对：**旧包**里 `dist/extension.js` 与 `dist/webview/grafana-instance-form.js` 都带 `sourceMappingURL` 而全包 `find -name '*.map'` 为空（悬空坐实），**新包** `grep -rl sourceMappingURL` **零命中**且同样不含 `.map`，13 个文件、目录结构与旧包逐项一致。**⑥ 产物不只量了体积，还冒烟跑过：** `node --check` 对两个 bundle 均通过；用打桩的 `vscode` 模块 `require` 解包后的压缩版 `extension.js`，输出 `exports: activate, deactivate`、两者 `typeof === 'function'`。另确认 `rg 'constructor\.name'` 在 `src/` 零命中、错误分支一律走 `instanceof` 与显式 `kind`/`status` 字段、`UserVisibleError` 的 `name` 是字符串字面量而非类名推导，故标识符 mangle 无行为可改。**⑦ watch 可调试性单独验证：** `npm run watch` 产出 `extension.js` 569,865 字节**未压缩**、`extension.js.map` 1,134,323 字节，两个 bundle 的 `sourceMappingURL` 均在位。**⑧ 全量（沙箱外，`required_permissions: ["all"]`）：** `npm run typecheck` 退出码 0、零诊断；`npm test` **35 文件 / 387 用例全部通过**（基线 373，本任务净增 14）。沙箱内另见 3 个 `EPERM: mkdir '.../.cursor'`（`McpConfigInstaller.test.ts`），与 P0-T3a / P1-B / P2-C / P3-G / P6-G 记录的同源，沙箱外全部消失，已实测确认。**⑨ 类型纪律：** `rg '\bas any\b|:\s*any\b|<any>|any\[\]|Array<any>|@ts-ignore|@ts-expect-error' src/` **0 匹配**；`rg '\bany\b' src/mcp/BridgeServer.ts` 唯一命中是注释里的英文单词（"would let any local process"）。新增代码未用 `as unknown as`（测试里为构造非字符串 header 用了一处 `42 as unknown as string`，属用例夹具，不在 `src/`） |
| 提交 | `29058e6`（鉴权前置 + socket 期限，2 文件）、`07eae8a`（常量时间比较 + `createBridgeToken`，3 文件）、`865857e`（生产构建压缩与 target，1 文件），另本条台账 1 条写入 hub 仓。均在分支 `chore/at-series-optimization-phase0`，**`git add` 与 `git commit` 两侧全部带 pathspec**，每次提交后以 `git show --stat` 复核文件数（2 / 3 / 1，与预期逐一相符）。未推送远程，未改 `package.json` / `package-lock.json`。两仓中他人未提交的在制品（grafana 仓 `package.json` / `package-lock.json` 两处真实改动与 20 余个仅 CRLF 差异的 ` M`、`?? docs/handoffs/`；hub 仓 ` M packages/mcp-hub/src/installer/cursor.ts` 等）全程未被波及 |

**为什么最终是「先答 401，让 Node 自己丢弃剩余 body」，而不是答完就掐断连接。** 一度打算给 401 加 `Connection: close`，让服务端明确不再收——但 `destroySoon()` 在**接收缓冲区里还有未读数据**时会以 RST 而不是 FIN 收场，对端可能连已经送达的 401 一起丢掉，测试与真实客户端都会变成偶发失灵。改为不动 keep-alive：Node 在响应结束时对未被消费的请求调 `req._dump()`，边到边丢。**内存量级因此从「无上限 × 2 MiB」变成「≤64 条连接 × 一个流缓冲（默认 16 KiB）」**，即最坏约 1 MiB 且需要占满连接数上限。攻击者仍能让内核和网卡替他搬字节，但搬不进扩展宿主的堆——放大效应没了，这才是这条要修的东西。

**`maxConnections` 是配套项而不是顺手加的。** 任务描述里的威胁是「2 MiB **× 无限并发**」，鉴权前置只解决了乘号左边。右边靠 `maxConnections: 64` 兜住：Bridge 唯一合法的客户端是本机 Hub，既不慢也不多，64 远高于任何正常用量，它是失控护栏而非调度策略。`connectionsCheckingInterval` 与 G5 在 `GrafanaEmbedProxy` 上踩过的坑同源——Node 不给每条连接挂定时器，两个超时由一个默认 30s 的周期扫描兑现，10s 的 headers 期限实际可能拖到 40s；不把扫描间隔与期限挂钩，配置的数字就是摆设。

**handler 内部那次 token 判定为什么保留而不是「挪」。** `createBridgeRequestHandler` 是可以脱离 Node 传输独立驱动的单元，`BridgeServer.test.ts` 与 `BridgeServer.integration.test.ts` 都直接调它。若把判定挪进 `handleNodeRequest`，handler 就变成一个「假定上游已鉴权」的组件，而它的类型签名完全不表达这个前提——下一个直接调用它的人不会知道。所以这是**在它前面加一道闸**：两处共用 `unauthorizedResponse()` 一个信封，两处都不依赖对方。既有 19 条 handler 级用例因此一条未改。

**`timingSafeEqualToken` 的两个边界都不是它自己兜的，调用方必须接住。** 它签名是 `(a: string, b: string)` 且两侧都过 `Buffer.from`，所以：**缺失的 header 必须以 `''` 而非 `undefined` 抵达**（`?? ''`），空串命中它「空 token 恒不匹配」的加固，判定为假；**非字符串必须在到达它之前被挡掉**，否则 `Buffer.from` 抛出、被 handler 的 catch 兜成 500——这一点是实测出来的（见「验证」④），不是预判。收窄放在 `headerValue` 而不是 `isAuthorized`，因为 `headerValue` 的返回类型本就写着 `string | undefined`，而 `value[0]` 在重复 header 为空数组时会违背它；让函数兑现自己的类型承诺，比在调用点补防御更不容易漏。

**`createBridgeToken()` 这条「非必须」的顺带项，判断是值得换。** 熵完全相同（都是 32 字节 CSPRNG），换的是「谁定义 Bridge token 是什么」：hex 是本仓手写的，base64url 是 hub 定义的。风险点只有一个——编码变了会不会有下游按 hex 校验，已逐条确认 `registry/read.ts` 对 `token` 只做 `isNonEmptyString`，无字符集与长度约束，且 base64url 字符集（`A-Za-z0-9_-`）本就是 HTTP header 安全的。配套的测试刻意**不写死 43 这个长度**，而是断言 `token.length === createBridgeToken().length`，这样 hub 若改编码，本仓测试跟着变而不是碎掉。

**任务 3 的悬空引用选择「不产出」而不是「产出后清理」。** 两条路都能消除悬空：要么 prod 不发 sourcemap，要么发了之后把 `//# sourceMappingURL` 从 bundle 里剪掉。后者需要一个后处理步骤，而它保护的东西是零——`.vscodeignore` 本来就不让 `.map` 进包，剪注释只是把「指向不存在的文件」换成「什么也不指」。直接 `sourcemap: watch` 一行同时表达了意图（map 是给 watch 模式调试用的）和结果，且 `.vscodeignore` 里那条 `**/*.map` 保留不动，仍然兜住 watch 之后忘记清 `dist/` 就打包的情形。

**两个 esbuild `target` 是从 `engines.vscode: ^1.85.0` 推出来的，不是拍的。** VS Code 1.85 = Electron 25，即扩展宿主 Node 18、Webview Chromium 114。esbuild 无 `target` 时按工具链自身能力发射语法，产出的东西可能在**本仓声明支持**的最老宿主上解析失败，而这种失败只在用户机器上出现，CI 与开发机永远看不到。本插件大量使用 `node:` API、`package.json` 无 `browser` 入口，Webview 只可能跑在 Electron 渲染进程里，因此 `chrome114` 就是「我们声称支持的最老宿主」而非保守猜测。

**两项如实记录的瑕疵。** 其一，`29058e6` 的提交信息正文里把 G5 那次 embed-proxy 超时改动的 sha 写成了 `671845b`，**正确的是 `671541b`**；提交已落盘且台账与提交信息都不该事后改写，故在此更正，以本条为准。其二，**一个范围外的观察**：早先在 `dist/` 里看到 `hub.js` 也带悬空的 `//# sourceMappingURL=hub.js.map`，但那是 15:29 的**陈旧副本**；开工前重建 hub 后重新 `copy:hub` 得到的 `hub.js`（390,267 字节）已不含该注释，新 VSIX 的全包 grep 零命中。该文件由 hub 仓的 `esbuild.hub.mjs` 产出、本仓只复制不构建，未做任何改动。
