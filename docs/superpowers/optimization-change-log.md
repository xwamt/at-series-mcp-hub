...
| 提交 | `366b4fb`（D1 risk → MCP annotations，含 `v2.md` §8），另台账 1 条。**D2 的四个文件（`src/hub/hostApp.ts`、`src/hub/main.ts`、`test/hub.hostApp.test.ts`、`docs/protocol/v1.md` §4.1）内容完整落在 `755e18f` 中——该 commit 归属并行的 subagent 2-C，成因是共享 git index 竞态，详见下方说明。** 均在分支 `chore/at-series-optimization-phase0`，**未推送远程、未 tag、未 publish、未改 package.json 版本号** |
...
**并发协作说明：与 2-C 发生一次 git index 竞态，D2 的四个文件被并入对方 commit（如实记录，未做历史改写）。** 本任务全程只 `git add` 明确路径，从未用 `.` / `-A` / `-u`；D2 的 `git add` 执行后 `git status` 复核也确认暂存区**恰好**是本任务的四个文件、对方的改动均在未暂存区。但在 `git add` 与 `git commit` 之间的窗口内，2-C 执行了自己的提交，把当时索引里的全部内容（含本任务四个文件）一并写进了 `755e18f`；本任务随后的 `git commit` 遂报 `no changes added to commit`。**已核验落地内容逐字正确**（`git show 755e18f -- <四个路径>` 的 diff 与本任务的编辑完全一致，无截断、无串改）。**刻意不做补救性的历史改写**：`755e18f` 是他人 commit 且非 HEAD 之外无从单独摘出，`--amend` / `rebase` / `reset` 在两个 subagent 仍在同分支活动时，损坏对方在制品的风险远高于「提交信息与内容不完全对应」这一记账瑕疵。**净效果是分支内容完全正确，仅 D2 的提交信息缺失**——`755e18f` 的 message 只描述了 2-C 的 token 工作。本条台账即为该缺失信息的归档：D2 的动机、实现与验证如上表所载。**建议后续并行任务把 `git add` 与 `git commit` 合并为单条 `&&` 命令以缩小竞态窗口**（本任务 D1 的 `366b4fb` 因两步之间无人插入而幸免，属运气而非设计）。除此之外与 2-A（`src/publisher/**`、`src/fs/**`）、2-C（`src/registry/**`、`src/protocol/**`、`src/index.ts`、`test/fixtures/fakeBridge.ts`）**无任何文件重叠**，全程未遇 `index.lock` 争用。
